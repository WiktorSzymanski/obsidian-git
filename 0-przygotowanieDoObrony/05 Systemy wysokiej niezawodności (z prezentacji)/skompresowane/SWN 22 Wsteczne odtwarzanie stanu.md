...
### Rodzaje awarii
Wyróżnia się cztery rodzaje awarii, różniące się przyczyną i możliwą reakcją:

- **Awaria procesu** — przyczyny: zakleszczenie, przekroczenie czasu oczekiwania, błąd ochrony, niespójność danych. Reakcja: wycofanie procesu (*abort*) lub restart przetwarzania.
- **Awaria węzła** — przyczyny: błędy programowe lub sprzętowe, zanik zasilania. Reakcja (jedyna sensowna): zatrzymanie węzła i restart ze zdefiniowanego wcześniej stanu.
- **Awaria pamięci masowej** — reakcja: rekonstrukcja danych z archiwum.
- **Awaria komunikacyjna** — przyczyny: uszkodzenie medium transmisyjnego albo urządzeń sieciowych. Reakcja: naprawa łącza i retransmisja utraconych komunikatów.

### Modele restartu w modelu fail-recovery
To, w jakim stanie system znajdzie się po restarcie, opisują trzy modele:

- **Przerwa** — system wznawia pracę w **dokładnie tym samym stanie**, który poprzedzał awarię; nic nie zostaje utracone.
- **Amnezja** — system startuje w **stanie predefiniowanym**, całkowicie niezależnym od tego, co działo się w chwili awarii.
- **Częściowa amnezja** — przypadek pośredni: część stanu pokrywa się ze stanem sprzed awarii, a pozostała część jest predefiniowana; tak zachowuje się na przykład serwer plików po restarcie.
...
Podejście to ma cztery istotne ograniczenia, które razem przesądzają o jego wąskiej stosowalności:

- **Zależność od trafności przewidywania błędów** — trzeba z góry wiedzieć, co może pójść źle i jaki będzie tego skutek.
- **Projektowanie pod konkretny system i konkretne błędy** — mechanizm nie przenosi się między zastosowaniami.
- **Bezużyteczność wobec błędów nieprzewidywalnych** — nie ma dla nich przygotowanej procedury naprawczej.
- **Brak możliwości implementacji jako ogólny mechanizm systemowy** (najważniejsze) — z powyższych powodów odtwarzanie postępowe zawsze pozostaje rozwiązaniem aplikacyjnym.
...
Za uniwersalność płaci się trzema kosztami:

- **Narzut** — bywa duży, bo trzeba systematycznie zapisywać stan lub historię przetwarzania.
- **Nawrót** — na ogół nie ma gwarancji, iż awaria nie wystąpi ponownie po odtworzeniu stanu.
- **Niepowtarzalność**, czyli niemożliwość wycofania — dotyczy tych komponentów, których po prostu nie da się cofnąć, przede wszystkim **interakcji ze światem zewnętrznym**, jak wydanie pieniędzy przez bankomat.
...
- **do** — wykonuje działanie i je rejestruje.
- **undo** — wycofuje je zgodnie z polem $UNDO$.
- **redo** — odtwarza je zgodnie z polem $REDO$.
...
W niektórych protokołach rozróżnia się dodatkowo dwa rodzaje punktów lokalnych:

- Punkt **ostateczny** (*permanent*) — należy do spójnego globalnego punktu kontrolnego i **tylko on może być punktem odtwarzania**.
- Punkt **wstępny** (*tentative*) — punkt, który jeszcze się ostatecznym nie stał.

Oba zapisywane są w pamięci trwałej, ale dla każdego procesu istnieje w danej chwili co najwyżej jeden punkt ostateczny.
...
Spójność pojedynczej pary zależy przy tym od kilku czynników naraz:

- **Determinizm** zdarzeń i przetwarzania.
- **Semantyka wiadomości**.
- Dostępność **dodatkowych mechanizmów** obsługujących duplikację, utratę wiadomości czy niespójność stanu kanałów.
...
W **checkpointingu skoordynowanym** (synchronicznym) procesy uzgadniają moment zapisu tak, aby powstający globalny punkt kontrolny **zawsze był $CP^\bullet$** — a w efekcie od razu stanowił najświeższą linię odtwarzania. Odtwarzanie jest dzięki temu trywialne, ale cena jest wysoka i sprowadza się do czterech wad:

- Wymaga **dodatkowych wiadomości kontrolnych**.
- Synchronizacja powoduje **duży narzut i opóźnienie**.
- Przy małej liczbie awarii prowadzi do **zbędnego ponoszenia tego kosztu**.
- Co najgorsze — koszt ten ponosi się **nawet wtedy, gdy w danym wykonaniu awarie w ogóle nie wystąpią**.
...
Możliwe są dwie drogi wyjścia:

- **Odszukanie spójnego punktu kontrolnego** wprost w zbiorze niezależnie wyznaczonych punktów.
- **Wycofanie się do punktu niekoniecznie spójnego** i wyznaczenie linii odtwarzania **z precyzją do pojedynczych zdarzeń**, odtwarzając je z rejestru utrzymywanego razem z punktami kontrolnymi — co prowadzi wprost do logowania komunikatów.
...
**Checkpointing quasi-synchroniczny** dąży do synchronizacji, ale niekoniecznie ją osiąga. Operuje dwoma rodzajami punktów kontrolnych:

- **Podstawowe punkty kontrolne** (*basic*) — każdy proces wyznacza je niezależnie, co ustalony interwał czasu.
- **Wymuszone punkty kontrolne** (*forced*) — implikowane komunikacją: powstają, gdy odebrana wiadomość niesie wyższy numer interwału niż bieżący numer odbiorcy.
...
### Miejsce logowania
Wiadomość można rejestrować po stronie odbiorcy, nadawcy albo obu naraz:

- **Rejestrowanie u odbiorcy** — uzasadnione tym, że operacja odbioru jest **niedeterministyczna**; odtworzenie wiadomości z rejestru **przyspiesza ponowne wykonanie** przetwarzania, a retransmisja zagubionej wiadomości może okazać się niepotrzebna.
- **Rejestrowanie u nadawcy** — usprawnia **retransmisję**, która i tak bywa potrzebna, umożliwia przetwarzanie **niedeterministyczne**, a przy rozgłaszaniu pozwala jednym zapisem obsłużyć wszystkie kopie wiadomości.

### Tryby logowania
Wyróżnia się cztery tryby rejestrowania:

- **Rejestrowanie pesymistyczne** — zapisuje log do pamięci trwałej **atomowo z operacją** wysłania lub odbioru; w praktyce kontroler zapisuje log **po** odebraniu, ale **przed** dostarczeniem wiadomości do procesu. Gwarantuje brak wiadomości osieroconych, ale okupione jest **dużym narzutem czasowym**, ponieważ blokuje przetwarzanie aplikacyjne.
...
- **Wybiórcze rejestrowanie pesymistyczne** — kompromis stosowany w checkpointingu quasi-synchronicznym: loguje się pesymistycznie, ale **tylko te wiadomości**, które prawdopodobnie trzeba będzie odtworzyć, jeśli proces wycofa się do punktu kontrolnego poprzedzającego ich odbiór.
...
Odrębnym problemem jest odróżnienie komunikatów sprzed awarii od tych wysłanych po cofnięciu procesu. Służy do tego **numer inkarnacji**, przesyłany z każdą wiadomością i **zwiększany po każdym odtworzeniu**; komunikaty oznaczone nieaktualnym numerem są odrzucane. Na tej podstawie definiuje się dwa rodzaje wiadomości wymagających odmiennego traktowania:

- Wiadomość **opóźniona** — gdy $m.inc\_num < inc\_num_i$ **i** $m.ckpt\_num < RL\_num_i$; jej wysłanie nie zostanie cofnięte, więc należy ją **przetworzyć**.
- Wiadomość **zduplikowana** — gdy $m.inc\_num < inc\_num_i$ **i** $m.ckpt\_num \geqslant RL\_num_i$; jej wysłanie zostanie cofnięte i powtórzone w nowej inkarnacji, więc należy ją **odrzucić**.

## Zależność punktów kontrolnych

### Relacja z-zależności
Punkt kontrolny $cp_j^{\,l}$ jest **bezpośrednio z-zależny** (*zigzag-dependent*) od $cp_i^{k}$ w dwóch przypadkach:

- **Kolejne punkty tego samego procesu** — $i = j \wedge l = k+1$.
- **Zależność komunikacyjna** — podczas interwału $I_i^{k}$ została wysłana wiadomość odebrana w interwale $I_j^{\,l-1}$, czyli formalnie:
...
Samo wyznaczanie linii odtwarzania przebiega iteracyjnie na zbiorze zwanym **root set**:

1. Początkowo root set zawiera **ostatnie punkty kontrolne wszystkich procesów**.
2. Oznacza się wszystkie punkty **osiągalne** z elementów tego zbioru.
3. Każdy oznaczony element zastępuje się **ostatnim nieoznaczonym** punktem tego samego procesu.
4. Gdy żaden element nie jest już oznaczony, **root set stanowi linię odtwarzania**.
